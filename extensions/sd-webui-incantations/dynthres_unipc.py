        ...
        **kwargs,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                ...
                if cbs != batch_size:
                    logger.warning(
                        "Got %s conditionings but batch-size is %s",
                        ctmp.shape[0],
                        batch_size,
                    )
        ...
        ns = uni_pc.uni_pc.NoiseScheduleVP(
            "discrete", alphas_cumprod=self.alphas_cumprod
        )
        model_type = "v" if self.model.parameterization == "v" else "noise"
        model_fn = CustomUniPC_model_wrapper(
        ...
        uni_pc_inst = uni_pc.uni_pc.UniPC(
            model_fn,
            ns,
            predict_x0=True,
            thresholding=False,
