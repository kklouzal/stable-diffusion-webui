...
def add_tab():
    with gr.Blocks(analytics_enabled=False) as ui:  # noqa: SIM117 - nested Gradio contexts make the tab hierarchy clearer.
        with gr.Row(equal_height=True):
            with gr.Column(variant="panel"):
                gr.HTML(
                    value="<p>OpenClaw-owned converter for checkpoints and LoRAs. Outputs are saved next to the source file.</p>"
                )
                mode = gr.Radio(
                    choices=["checkpoint", "lora"], value="checkpoint", label="Mode"
                )
                with gr.Row():
                ...
                lora_name = gr.Dropdown(
                    [item["title"] for item in convert.list_loras()], label="LoRA"
                )
                lora_precision = gr.Radio(
                    choices=["fp32", "fp16", "bf16"],
                    value="bf16",
                    label="LoRA precision",
                )
                ...
                with gr.Row():
                    ...
                    m_type = gr.Radio(
                        choices=["disabled", "no-ema", "ema-only"],
                        value="disabled",
                        label="Pruning Methods",
                    )

                with gr.Row():
                    checkpoint_formats = gr.CheckboxGroup(
                        choices=["ckpt", "safetensors"],
                        value=["safetensors"],
                        label="Checkpoint Format",
                    )
                    show_extra_options = gr.Checkbox(
                        label="Show part actions", value=False
                    )

                with gr.Row():
                    bake_in_vae = gr.Dropdown(
                ...
                with gr.Row():
                    force_position_id = gr.Checkbox(
                        label="Force CLIP position_id to int64 before convert",
                        value=True,
                    )
                    fix_clip = gr.Checkbox(label="Fix CLIP", value=False)
                    delete_known_junk_data = gr.Checkbox(
                        label="Cleanup known junk", value=False
                    )

                with gr.Row(visible=False) as extra_options:
                    specific_part_conv = ["copy", "convert", "delete"]
                    unet_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="UNet"
                    )
                    text_encoder_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="Text encoder / CLIP"
                    )
                    vae_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="VAE"
                    )
                    others_conv = gr.Dropdown(
                        specific_part_conv, value="convert", label="Other weights"
                    )
                ...
                with gr.Row(visible=False) as precision_options:
                    ...
                    unet_precision = gr.Dropdown(
                        specific_precision, value="inherit", label="UNet precision"
                    )
                    clip_precision = gr.Dropdown(
                        specific_precision, value="inherit", label="CLIP precision"
                    )
                    vae_precision = gr.Dropdown(
                        specific_precision, value="inherit", label="VAE precision"
                    )
                    other_precision = gr.Dropdown(
                        specific_precision, value="inherit", label="Other precision"
                    )

                model_converter_convert = gr.Button(
                    value="Convert model",
                    elem_id="model_converter_convert",
                    variant="primary",
                )

            with gr.Column(variant="panel"):
                submit_result = gr.Textbox(
                    elem_id="model_converter_result", show_label=False
                )

            show_extra_options.change(
                fn=lambda x: (gr_show(x), gr_show(x)),
                inputs=[show_extra_options],
                outputs=[extra_options, precision_options],
            )

            model_converter_convert.click(
                fn=lambda mode, model_name, lora_name, lora_precision, checkpoint_formats, precision, m_type, custom_name, bake_in_vae, unet_conv, text_encoder_conv, vae_conv, others_conv, unet_precision, clip_precision, vae_precision, other_precision, fix_clip, force_position_id, delete_known_junk_data: (
