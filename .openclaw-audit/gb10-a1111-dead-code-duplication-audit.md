...
## Pass 117 - Sampler wrapper/runtime handoff code (2026-06-21)

### Scope checked
- `modules/sd_samplers.py`: sampler registry assembly, `find_sampler_config()`, `create_sampler()`, `set_samplers()`, visible sampler helpers, infotext sampler/scheduler parsing, hires sampler/scheduler normalization, and invalid sampler/scheduler autocorrection.
- `modules/sd_samplers_common.py`: `setup_img2img_steps()`, VAE preview/sample conversion adjacency, ETA noise seed delta helper, torchsde Brownian override, refiner switch/reload handoff, `TorchHijack`, base `Sampler.initialize()`, `create_noise_sampler()`, `launch_sampling()`, callback state, and infotext padding markers.
- `modules/sd_samplers_kdiffusion.py`: k-diffusion sampler data/options, `sampler_extra_params`, `CFGDenoiserKDiffusion.inner_model`, `KDiffusionSampler.get_sigmas()`, txt2img/img2img sampling handoff, scheduler metadata emission, brownian/noise sampler handling, solver type handling, and extra-arg assembly.
- `modules/sd_samplers_timesteps.py`: DDIM/PLMS/UniPC sampler data, timestep denoiser wrappers, `CFGDenoiserTimesteps`, `_make_timesteps()`, `CompVisSampler.get_timesteps()`, txt2img/img2img timestep sampling handoff, and legacy `sd_samplers_compvis` / `VanillaStableDiffusionSampler` compatibility aliases.
- Adjacent tests/references: `test/test_openclaw_multi_sampler.py`, `test/test_openclaw_schedulers.py`, `tests/test_processing_auxiliary_infotext_alignment.py`, `modules/sd_samplers_cfg_denoiser.py` sampler-extra-arg mutation/read sites, `modules/sd_schedulers.py`, and exact reference scans for sampler runtime handoff names and compatibility aliases.

### Findings / fixes
- Consolidated duplicate sampler `extra_args` assembly. Added base `Sampler.set_sampler_extra_args()` and reused it from k-diffusion txt2img, k-diffusion img2img, timestep txt2img, and timestep img2img paths, preserving the exact `cond`, `image_cond`, `uncond`, `cond_scale`, and `s_min_uncond` keys and keeping `self.sampler_extra_args` as the mutable dict that CFG denoiser schedule reconstruction updates.
- Preserved k-diffusion scheduler/sigma logic. `get_sigmas()` still handles penultimate sigma discard options, Automatic/default scheduler selection, sampler noise scheduler overrides, sigma min/max/rho infotext, beta scheduler parameters, inner-model schedulers, and CPU-returned sigmas; these branches are option/model driven and not dead code.
- Preserved timestep schedule logic. `_make_timesteps()` and `CompVisSampler.get_timesteps()` remain separate from k-diffusion sigma scheduling because DDIM/PLMS/UniPC denoisers consume integer timestep tensors and alphas, not sigma schedules.
- Preserved compatibility aliases `sys.modules['modules.sd_samplers_compvis']` and `VanillaStableDiffusionSampler`. They are extension-facing import/class compatibility surfaces even though the in-repo implementation now lives in `sd_samplers_timesteps.py`.
- Preserved sampler names, aliases, options, refiner switch/reload handling, live-preview latent storage, ETA/ENSD behavior, Brownian noise determinism, callback state updates, and torch hijack behavior. No safe stale public sampler wrapper or scheduler alias removal was proven in this slice.

### Static/dynamic audit map notes
- Runtime handoff remains: processing selects sampler -> sampler `initialize()` seeds eta/sigma options and torch hijack -> sampler-specific schedule/timestep setup -> `set_sampler_extra_args()` establishes CFG denoiser arguments -> sampler function calls CFG denoiser with `extra_args=self.sampler_extra_args` -> CFG denoiser may mutate `cond`/`uncond` in the same dict as prompt schedules advance.
- K-diffusion and timestep sampler paths intentionally keep distinct schedule builders and img2img noise application because k-diffusion operates in sigma space while CompVis timestep samplers operate against `alphas_cumprod` timestep indexes.
- Compatibility surfaces kept conservative: sampler names/aliases/options, scheduler labels and Automatic behavior, `sd_samplers_compvis`, `VanillaStableDiffusionSampler`, refiner hooks, live preview state, extension callback hooks, ETA/noise scheduler metadata, and img2img extra-noise callback behavior.

### Validation log
- `PYTHONPYCACHEPREFIX=$(mktemp -d /tmp/gb10-a1111-pycompile-pass117.XXXXXX) python3 -m py_compile modules/sd_samplers.py modules/sd_samplers_common.py modules/sd_samplers_kdiffusion.py modules/sd_samplers_timesteps.py modules/sd_schedulers.py test/test_openclaw_multi_sampler.py test/test_openclaw_schedulers.py tests/test_processing_auxiliary_infotext_alignment.py` - passed.
- `PYTHONPYCACHEPREFIX=$(mktemp -d /tmp/gb10-a1111-pytest-pass117.XXXXXX) python3 -m pytest -q test/test_openclaw_multi_sampler.py test/test_openclaw_schedulers.py tests/test_processing_auxiliary_infotext_alignment.py` - blocked during collection because the system `python3` environment cannot import `torch` (`ModuleNotFoundError: No module named 'torch'`), with the existing pytest config warning `Unknown config option: base_url`.
- `PYTHONPYCACHEPREFIX=$(mktemp -d /tmp/gb10-a1111-pytest-pass117-processing.XXXXXX) python3 -m pytest -q tests/test_processing_auxiliary_infotext_alignment.py` - passed: 10 passed, with the existing pytest config warning `Unknown config option: base_url`.
- Exact reference scan confirmed sampler extra-arg handoff is now built by `set_sampler_extra_args()` and still consumed/mutated by `modules/sd_samplers_cfg_denoiser.py`; compatibility aliases remain only in `modules/sd_samplers_timesteps.py` and were preserved as public extension surfaces.
- Exact AST duplicate-body scan across `modules/sd_samplers.py`, `modules/sd_samplers_common.py`, `modules/sd_samplers_kdiffusion.py`, `modules/sd_samplers_timesteps.py`, and `modules/sd_schedulers.py` reported no duplicate nontrivial function/class bodies.
- `git diff --check` - passed.
- Live WebUI/API startup, CUDA sampler execution, actual k-diffusion/timestep image equivalence, real refiner handoff, Brownian determinism under batch variation, and extension imports of legacy sampler aliases were not exercised in this bounded slice.
