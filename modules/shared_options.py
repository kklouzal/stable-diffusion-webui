...
from modules import ui_components, shared_items, shared, interrogate, util, sd_emphasis, torchao_weight_quant
...
options_templates.update(options_section(('system', "System", "system"), {
    ...
    "show_gradio_deprecation_warnings": OptionInfo(True, "Show UI deprecation warnings in console.").needs_reload_ui(),
...
options_templates.update(options_section(('optimizations', "Optimizations", "sd"), {
    ...
    "persistent_img2img_init_cache": OptionInfo(True, "Persistent img2img init cache").info("reuse VAE-encoded img2img init image/mask conditioning when exact inputs and relevant settings have not changed"),
...
options_templates.update(options_section(('ui_gallery', "Gallery", "ui"), {
    ...
    "sd_webui_modal_lightbox_icon_opacity": OptionInfo(1, "Full page image viewer: control icon unfocused opacity", gr.Slider, {"minimum": 0.0, "maximum": 1, "step": 0.01}).info('for mouse only').needs_reload_ui(),
    "sd_webui_modal_lightbox_toolbar_opacity": OptionInfo(0.9, "Full page image viewer: tool bar opacity", gr.Slider, {"minimum": 0.0, "maximum": 1, "step": 0.01}).info('for mouse only').needs_reload_ui(),
...
options_templates.update(options_section(('ui', "User interface", "ui"), {
    "localization": OptionInfo("None", "Localization", gr.Dropdown, {"choices": ["None"]}).needs_reload_ui(),
    ...
    "ui_tab_order": OptionInfo([], "UI tab order", ui_components.DropdownMulti, {"choices": []}).needs_reload_ui(),
    "hidden_tabs": OptionInfo([], "Hidden UI tabs", ui_components.DropdownMulti, {"choices": []}).needs_reload_ui(),
    ...
    "gradio_theme": OptionInfo("Default", "Legacy UI theme", ui_components.DropdownEditable, {"choices": ["Default"]}).info("you can also manually enter compatible theme names from the legacy theme gallery.").needs_reload_ui(),
    "gradio_themes_cache": OptionInfo(True, "Cache legacy UI themes locally").info("disable to update the selected UI theme"),
