...
def _sd3_preview_factors(device, dtype):
    return torch.as_tensor(_SD3_PREVIEW_FACTORS, device=device, dtype=dtype)
...
class SD3LatentFormat:
    ...
    def decode_latent_to_preview(self, x0):
        ...
        latent = x0[0].permute(1, 2, 0).to(dtype=torch.float32)
        latent_image = latent @ _sd3_preview_factors(latent.device, latent.dtype)
