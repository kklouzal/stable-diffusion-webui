...
parser.add_argument("--no-progressbar-hiding", action='store_true', help="do not hide progressbar in the legacy browser UI (we hide it because it slows down ML if you have hardware acceleration in browser)")
...
parser.add_argument("--share", action='store_true', help="legacy compatibility option for hosted UI sharing")
parser.add_argument("--ngrok", type=str, help="ngrok authtoken, legacy alternative to --share", default=None)
...
parser.add_argument("--listen", action='store_true', help="launch server with 0.0.0.0 as server name, allowing it to respond to network requests")
parser.add_argument("--port", type=int, help="launch server with given port; ports < 1024 require root/admin rights; defaults to 7860 if available", default=None)
...
parser.add_argument("--gradio-debug",  action='store_true', help="legacy compatibility option for UI debug mode")
parser.add_argument("--gradio-auth", type=str, help='set legacy UI authentication like "username:password"; or comma-delimit multiple like "u1:p1,u2:p2,u3:p3"', default=None)
parser.add_argument("--gradio-auth-path", type=normalized_filepath, help='set legacy UI authentication file path ex. "/path/to/auth/file" same auth format as --gradio-auth', default=None)
parser.add_argument("--gradio-img2img-tool", type=str, help='does not do anything')
parser.add_argument("--gradio-inpaint-tool", type=str, help="does not do anything")
parser.add_argument("--gradio-allowed-path", action='append', help="add path to legacy UI allowed_paths, making it possible to serve files from it", default=[data_path])
...
parser.add_argument("--no-gradio-queue", action='store_true', help="legacy compatibility option for disabling the UI queue")
parser.add_argument("--skip-version-check", action='store_true', help="Do not check the torch version")
parser.add_argument("--no-hashing", action='store_true', help="disable sha256 hashing of checkpoints to help loading performance", default=False)
parser.add_argument("--no-download-sd-model", action='store_true', help="don't download SD1.5 model even if no model is found in --ckpt-dir", default=False)
parser.add_argument('--subpath', type=str, help='customize the server subpath, for reverse proxy compatibility')
