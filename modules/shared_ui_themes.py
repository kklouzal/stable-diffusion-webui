...
def reload_ui_theme(theme_name=None):
    if not theme_name:
    ...
    if theme_name == "Default":
        ...
        try:
            ...
            errors.display(e, "changing UI theme")
            shared.gradio_theme = gr.themes.Default(**default_theme_args)

    # append additional values to the selected UI theme
...
def resolve_var(name: str, ui_theme=None, history=None):
    """
    ...
    Parameters:
        ...
        ui_theme: The theme object to resolve the variable from
    ...
    try:
        ...
        if value := getattr(ui_theme, name, None):
            return resolve_var(value, ui_theme, history + [name])
        else:
            return name
